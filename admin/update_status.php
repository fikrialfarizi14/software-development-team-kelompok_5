...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $pesanan) {
    ...
    $catatan_baru = bersihkan($_POST['catatan']);
    ...
    if (in_array($status_baru, $valid_status)) {
        ...
        // Catat perubahan ini ke riwayat tracking_status, supaya pelanggan bisa lihat timeline-nya
        catatTracking($id, $status_baru, $catatan_baru);
...
// Cek apakah pesanan ini sudah punya catatan pembayaran
$sudah_bayar = false;
if ($pesanan) {
    $cekBayar = mysqli_query($koneksi, "SELECT status_bayar FROM transaksi WHERE pesanan_id = $id");
    if ($row = mysqli_fetch_assoc($cekBayar)) {
        $sudah_bayar = $row['status_bayar'] === 'lunas';
    }
}
?>
<!DOCTYPE html>
...
<div class="container">
    ...
    <div class="card">
        ...
        <table style="margin-bottom:20px;">
            ...
            <tr><th>Catatan Pelanggan</th><td><?= $pesanan['catatan'] ?: '-' ?></td></tr>
            <tr><th>Status Sekarang</th><td><?= labelStatus($pesanan['status']) ?></td></tr>
            <tr><th>Pembayaran</th><td><?= labelBayar($sudah_bayar ? 'lunas' : 'belum_bayar') ?>
                &nbsp; <a href="transaksi.php?pesanan_id=<?= $id ?>" class="btn btn-sm btn-secondary">Kelola Pembayaran</a>
            </td></tr>
        </table>
        ...
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <textarea name="catatan" placeholder="Contoh: cucian sudah kering, siap disetrika..."></textarea>
            </div>
